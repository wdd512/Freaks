...
export function MouthLayer({ spec }: ArtLayerProps) {
  ...
  switch (name) {
    ...
    default: mouth = <g><rect x="55" y={y - 2} width="20" height="8" fill="#3a1d21" /><rect x="58" y={y - 1} width="14" height="3" fill={white} /><rect x="60" y={y + 2} width="3" height="2" fill={outline} /><rect x="67" y={y + 2} width="3" height="2" fill={outline} /></g>;
  }
  return <g data-layer="mouth">{mouth}{spec.presentation.mouthPose === "SMILE_ACCENT" && <><rect x="53" y={y - 1} width="2" height="2" fill={red} opacity="0.7" /><rect x="75" y={y - 1} width="2" height="2" fill={red} opacity="0.7" /></>}{spec.presentation.mouthPose === "MELTDOWN_ACCENT" && name !== "Cigarette" && <><rect x="53" y={y + 5} width="3" height="2" fill={red} /><rect x="74" y={y + 5} width="3" height="2" fill={red} /></>}</g>;
}
