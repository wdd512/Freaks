...
- `components/` contains UI, responsive SVG charts, and deterministic layered SVG Freak artwork.
...
More detail lives in [GAME_ENGINE.md](docs/GAME_ENGINE.md), [DATA_MODEL.md](docs/DATA_MODEL.md), and [GENERATION.md](docs/GENERATION.md).

## V0 limitations

- Mock prices are a deterministic game feed, not historical or live exchange data.
- The SVG character renderer is an intentional replaceable placeholder for production layered artwork.
...
After playtesting the economy and career pacing, the next phase is a production repository/worker deployment, real layered art, optional read-only live market adapter with durable price snapshots, season operations, and image export. Blockchain work should remain separate until the game is fun and the deterministic engine is stable.
