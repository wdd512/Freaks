...
export function WorkstationLayer({ spec }: ArtLayerProps) {
  ...
  switch (name) {
    ...
    case "Institutional Desk": base = <><rect x="3" y={y} width="122" height="9" fill="#252c2b" stroke={gold} strokeWidth="2" /><rect x="9" y={y + 9} width="110" height="18" fill="#121817" stroke="#56605c" strokeWidth="2" />{[18, 34, 50, 66, 82, 98].map((x) => <rect key={x} x={x} y={y + 14} width="8" height="3" fill={x % 3 ? spec.palette.green : spec.palette.red} />)}</>; break;
    case "Old Laptop Desk": base = <><rect x="10" y={y} width="108" height="7" fill="#74533b" stroke={outline} strokeWidth="3" /><rect x="44" y={y - 25} width="40" height="23" fill="#323936" stroke={outline} strokeWidth="3" /></>; break;
    case "Single Monitor Desk": base = <><rect x="7" y={y} width="114" height="8" fill="#57463a" stroke={outline} strokeWidth="3" /><rect x="39" y={y - 39} width="50" height="35" fill="#121a19" stroke={metal} strokeWidth="3" /></>; break;
    case "Dual Monitor Setup": base = <><polygon points={`4,${y} 124,${y} 117,${y + 10} 11,${y + 10}`} fill="#252a2b" stroke={purple} strokeWidth="3" /><rect x="18" y={y - 37} width="43" height="32" fill="#111918" stroke={metal} strokeWidth="3" /><rect x="67" y={y - 40} width="45" height="35" fill="#111918" stroke={metal} strokeWidth="3" /></>; break;
    case "Executive Trading Setup": base = <><rect x="3" y={y} width="122" height="10" fill="#202626" stroke={gold} strokeWidth="3" /><rect x="9" y={y + 10} width="110" height="24" fill="#111615" stroke="#545e5a" strokeWidth="3" /></>; break;
    default: base = assertNever(name, "workstation");
  }
