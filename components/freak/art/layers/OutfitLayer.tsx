...
export function OutfitLayer({ spec }: ArtLayerProps) {
  ...
  switch (spec.dynamic.outfit) {
    ...
    default: detail = <><rect x={x - 2} y={y} width={w + 4} height={h} fill="#25242c" /><path d={`M${x} ${y}L${x + w / 2} ${y + 18}L${x + w} ${y}`} fill="none" stroke="#625b6d" strokeWidth="5" /><rect x={x + 5} y={y + 23} width={w - 10} height="3" fill={gold} /><rect x={x + 7} y={y + 28} width="5" height="5" fill={gold} /></>;
  }
